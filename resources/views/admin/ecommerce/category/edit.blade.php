        ...
        <div class="row">
            <div class="col-xs-12 col-md-6 col-xs-offset-0 col-md-offset-3">
                <div class="app-container">
                    <h3 class="page-header">Edit Product Category</h3>

                    <form action="{{url('admin/product/category/update')}}" method="post" role="form">
                        {{csrf_field()}}
                        ...
                        <div class="form-group{{ $errors->has('parent_id') ? ' has-error' : '' }}">
                            ...
                            <select name="parent_id" id="parent_id" class="form-control">
                                ...
                                @foreach(\App\Models\ECommerce\ProductCategory::nestedToTabs() as $row)
                                    <option value="{{$row->id}}" {{is_selected($row->id, $product_category->parent_id)}}>{{$row->name}}</option>
                                @endforeach
                            </select>
